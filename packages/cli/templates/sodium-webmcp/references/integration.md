...
`init` creates `.sodium/project.json` as `null` so the application remains buildable before its first deployment. Treat the import as `SodiumProject | null`. While it is null, the SDK registers no tools and sends no telemetry. `deploy` replaces it with public project credentials and a signed receipt for the exact compiled contract and origins. Editing `sodium.json` invalidates that receipt until the next deploy. Do not invent credentials or edit this file yourself.
