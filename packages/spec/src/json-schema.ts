...
/**
 * The constrained JSON Schema dialect allowed in tool contracts. Deliberately
 * small: no $ref, no combinators, no remote anything. The browser SDK validates
 * inputs against the same shape with a
...
/**
 * The desugared root schema: what the SDK registers with `registerTool` and
 * validates agent input against. Always closed (`additionalProperties: false`)
 * so an agent cannot smuggle extra keys into a handler.
 */
export interface ToolInputSchema extends JsonSchemaSubset {
  type: "object";
  properties: Record<string, JsonSchemaSubset>;
  required: string[];
...
/**
 * Deterministic value validation against the subset. Used by strict config
 * validation and tests that cross-check the browser validator.
 */
