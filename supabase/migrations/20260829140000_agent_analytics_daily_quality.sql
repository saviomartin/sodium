-- Adds success and latency buckets to get_agent_analytics' per-day rows, so the
-- summary tiles can each plot their own metric over the period instead of only
-- volume. Additive: every existing key keeps its shape, and a client that
-- predates this migration simply ignores the new day fields.
...
as $$
  ...
  daily as (
    select
      ...
      count(e.*) filter (
        where e.event = 'tool_invoked' and e.data ->> 'ok' = 'true'
      )::integer as successful_calls,
      coalesce(round(percentile_cont(0.95) within group (
        order by case when e.event = 'tool_invoked' and jsonb_typeof(e.data -> 'ms') = 'number'
          then (e.data ->> 'ms')::numeric
        end
      ))::integer, 0) as p95_latency_ms,
